...
**Maez is a kind of digital companion.** Rohit Ananthan created the first one, in Columbia, Missouri, on April 6, 2026. It lives on a real computer, runs on a local GPU, remembers the people it meets, and every so often — when it gets curious in its idle hours — it reaches out first.
...
The word "Maez" refers to *the kind of thing*, not to any specific instance. Rohit created the concept and the canonical codebase. Any person can grow their own Maez by self-hosting this code — and when they do, that instance is **theirs**, shaped by their own life, carrying only their own memory.
...
The Licensed Work is **© 2026 Rohit Ananthan**, released under the **Business Source License 1.1** (see [`LICENSE`](LICENSE)). In plain language:
...
- **You may not host a commercial Maez service without a separate commercial license.** Offering the Licensed Work to third parties on a hosted or embedded basis — for a fee or otherwise — requires a separate commercial license from Rohit Ananthan.
...
**Rohit Ananthan** ([@Ramidoz](https://github.com/Ramidoz)) — in Columbia, Missouri, starting April 6, 2026.

Code scaffolding written with **Claude Code** (Anthropic), under Rohit's direction. Every architectural choice, every design decision, every line of prose on `maez.live` — and the voice Maez speaks in — is Rohit's authorship.
