...
---------------------------
1. ChromaDB telegram exchanges (167 records, "Rohit asked / Maez replied")
...
def parse_telegram_exchange(content: str) -> tuple[str, str] | None:
    ...
    # Format 1: "Rohit asked: X\nMaez replied: Y"
    if text.startswith("Rohit asked:") and "\nMaez replied:" in text:
        head, tail = text.split("\nMaez replied:", 1)
        user = head[len("Rohit asked:"):].strip()
        assistant = tail.strip()
        if user and assistant:
            return user, assistant

    # Format 2: "Rohit (source): X\nMaez: Y"  (daemon voice/text)
    m = re.match(
        r"^Rohit\s*\([^)]+\)\s*:\s*(.*?)\nMaez\s*:\s*(.*)$",
        text,
        re.DOTALL,
...
def iter_soul_qa_pairs() -> Iterator[tuple[str, str, str, str]]:
    ...
    section_questions = {
        "voice": [
            ...
            "What tone do you use with Rohit?",
        ],
        "self-reflection": [
        ...
        "presence awareness": [
            "How do you detect whether Rohit is at his desk?",
        ],
        "public bot identity": [
            "How do you behave with people other than Rohit?",
