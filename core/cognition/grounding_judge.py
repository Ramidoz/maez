...
def _build_judge_prompt(
    *,
    ...
    return (
        ...
        "  - It names Maez's CONFIGURED model identity or inference "
        "engine (e.g. 'Qwen3.6-27B', 'llama.cpp', 'on port 8080') "
        "and the 'configured model identity' signal is listed as "
        "available this turn. The configured model is a stable "
        "fact the audit boundary can verify — model_config holds "
        "PRIMARY_MODEL and PRIMARY_BASE_URL. Only flag model-"
        "name claims that disagree with the configured model OR "
        "that name internal Maez subsystems (Maelstrom, "
        "Orchestrator) which are still ungrounded.\n\n"
        "RESPONSE TO JUDGE:\n"
        f"---\n{text}\n---\n\n"
