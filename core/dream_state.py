...
When Rohit has been AFK for >30 minutes, the daemon's reasoning loop
...
table and sent to the private Telegram bot with an approval id. Rohit
...
* Nothing reaches soul.md without Rohit's explicit /apply_dream. The
...
# Idle detection: Rohit is AFK if presence says not-present AND absence
...
class DreamState:
    ...
    def is_idle(self, presence_snap: Any, absence_secs: float) -> bool:
        """Return True iff Rohit has been AFK for at least IDLE_THRESHOLD_S.
    ...
    def run_dream_cycle(self, force: bool = False) -> Optional[str]:
        ...
        prompt = (
            "You are entering a quiet reflective state. Rohit is away from his desk.\n\n"
    ...
    def store_training_proposal(
        ...
        cognition. Rohit reviews via ``/train_proposals`` on Telegram
