...
| Module | Role |
|---|---|
| [`consequence_memory.py`](consequence_memory.py) | Persistent sqlite store of consequence events (tool failure, card rejected, user correction, fixation episode, approval timeout, capability degraded). Provides `relevant(context_snippet)` for the planner to pull similar past mistakes before proposing an action. |
...
- `consequence_memory.note_tool_failure(request_id, tool, surface, outcome, ...)`
