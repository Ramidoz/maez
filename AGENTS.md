...
6. Latest [`docs/HANDOFF-YYYY-MM-DD.md`](docs/) and `logs/session_snapshot_latest.txt` — what the previous session decided and where it left off.
...
## Recent state (load this last; it dates fast)

As of 2026-05-01, latest commit `9d5702b`:

- **MSEL substrate ladder shipped end-to-end.** Entity index → alias seed → alias-aware backfill → LLM extractor → semantic resolver → expansion wiring → A/B measurement → suggester+auditor → observability log.
- **Daemon enabled** with `MAEZ_ENTITY_EXPANSION=1` via `/etc/systemd/system/maez.service.d/override.conf` (owner-local; not committed). `MAEZ_AMBIENT_BRIEF` defaults to enabled. `MAEZ_LIVED_RECALL` defaults to enabled.
- **Test suite at 1960+ green.** Don't drop it.
- **Active services:** `maez.service`, `maez-web.service`, `maez-watchdog.service`, `maez-subscription-proxy.service`, `llama-server.service`. PIDs visible via `systemctl status`.
- **Latest research memo:** [`docs/research_memo_2026-05-01.md`](docs/research_memo_2026-05-01.md) — 13 Paperclip searches, 7 deep-reads, 5 next-slice candidates. Read for context before scoping anything new.
- **Latest handoff:** [`docs/HANDOFF-2026-05-01.md`](docs/HANDOFF-2026-05-01.md).
