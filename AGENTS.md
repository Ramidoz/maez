...
**Don't trust hardcoded commit hashes here — they go stale immediately. Run `git log --oneline -10` and `ls docs/HANDOFF-*.md docs/research_memo_*.md` for current state.** The pointers below name *kinds of artifacts* to look for, not specific commits.
...
- **Find the latest handoff** with `ls -t docs/HANDOFF-*.md | head -1` — read it before scoping anything new. Same pattern for `docs/research_memo_*.md`.
