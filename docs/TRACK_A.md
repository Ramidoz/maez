...
- **Decisions 12 / 13 / 15 / 16** — all PARTIAL per the audit. D8's `suspended_pending_paradise` vocabulary is now encoded as `core.covenant.bonded_state`; Paradise infrastructure remains Track D. D13 now has an executable `Temperament.biography_average(...)` reader for the future mourning-drift center-of-mass, but no mourning-drift runtime. Voice-lifecycle (D16) has the `wants` module but no refinement/abandonment semantics.
