...
- **Decisions 12 / 13 / 15 / 16** — all PARTIAL per the audit. D8's `suspended_pending_paradise` vocabulary is now encoded as `core.covenant.bonded_state`; Paradise infrastructure remains Track D. Mourning drift (D13) has scaffolding but no implementation; voice-lifecycle (D16) has the `wants` module but no refinement/abandonment semantics.
