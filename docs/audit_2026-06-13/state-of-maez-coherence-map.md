...
2. **Egress rails computed-but-not-enforced at the cloud boundary** — `origin_downgrade` (the laundering case the gate exists to catch) is detected then **forwarded** at `core/subscription_proxy/server.py:747-771`, while Telegram enforces all blocks. Same gate, two trust levels. A mislabeled-safe private span can reach the cloud model.
3. **The S7 soul-write door is a dead-end through its own bridge** — the bridge hands a `127.0.0.1` pointer (`s7_ceremony_bridge.py:127`) while the verifier hardcodes `localhost` (`core/governance/s7_webauthn_ceremony.py:319-320`), so the hardware-key proof fails origin validation. The cockpit has **no systemd unit** (dead after every reboot). And `operator_user_boundary.py:3762-3808` still narrates S7.1 as "not mounted" while it's live — **the canonical authority file lies about the live body.**
...
9. **The Build Ledger violates its own maintenance law** — **(owner-corrected:** the `S7 ceremony bridge v0` row is correctly `BUILT_ASLEEP`; it's the adjacent `S7 soul affected-ref derivation (Task 0.5)` row that was stale at `BUILT_ON_BRANCH` after the merge to main — now fixed @b587d50). The broader point stands: the ledger omits 10+ live organs and competes with the runtime `capability_card` registry, so it isn't yet trustworthy as a whole-body chart.
...
| # | Slice | Risk / size |
|---|---|---|
| 1 | **Enforce `origin_downgrade` (+ the other computed-but-forwarded blocks) at the cloud chokepoint** (`core/subscription_proxy/server.py:747-771`), behind a shadow kill-switch, mirroring Telegram. +test asserting 403. | low / 1 file+test |
