...
**Architecture:** `core/memory/narrative.py` (NarrativeStore over `lived_episodes.db`: `narrative_links` + `narrative_proposals` tables; pure detectors; coverage) + hook in `EpisodeStore.add` callers + weave proposer/promoter + reflection thread-diet + `scripts/narrative_spine.py` + dormant recall/presence seams. Flags: `MAEZ_NARRATIVE_SPINE`, `MAEZ_NARRATIVE_WEAVE`, `MAEZ_NARRATIVE_REFLECTION`, `MAEZ_NARRATIVE_RECALL`, `MAEZ_NARRATIVE_PRESENCE`. Flag-off byte-identical at every layer.
...
- `same_thread`: shared joinable-class citation (evidence: the shared ids) + **scar-sidecar tokens**: episodes sharing a sidecar `dedup_key` (evidence: the dedup_key + sidecar row).
- `because_of`: typed hooks only — v0 hook list: (a) scar-recurrence order via sidecar (`prior_episode_ids` → newer scar `because_of` prior, hook_class=`scar_recurrence`); (b) scar episode → episode sharing its triggering receipt id when class semantics name that receipt as cause (hook_class=A1 scar class). Nothing else.
`DETECTOR_VERSION="v0"`.

- [ ] RED fixtures FROM THE CENSUS: the 43-reflection blob fixture yields ZERO same_thread (only strings) — the anti-blob test; two episodes sharing a raw uuid → one same_thread with those ids as evidence; core-row co-citation → nothing; sidecar dedup-key pair → same_thread + because_of(scar_recurrence) in supersession order; no hook → no because_of. — [ ] GREEN → Commit.

## Task 3: live hook + owner-gated backfill (flag `MAEZ_NARRATIVE_SPINE`)

**Files:** Modify the episode-add seam (writers call a `narrative_on_episode_added(episode)` helper — additive, flag-gated, fail-safe try/except so narrative can never break episode writes); Create `scripts/narrative_backfill.py`; Tests `tests/test_narrative_hook.py`, `tests/test_narrative_backfill.py`.

- [ ] RED: flag-off → helper never invoked (patched-to-fail test) + episode add byte-identical; flag-on → new episode's links written with evidence; hook exception → episode write unaffected. Backfill: `list` (no mutation; prints link counts by type — expected at birth: strings≈207, same_thread 0) and `apply --owner-approved`; **run twice → links table byte-identical**; idempotent with the live hook both on. — [ ] GREEN → Commit.
...
## Self-Review
**Spec coverage:** all three durable types + exhaustiveness (T1 CHECK + T8 guard); joinability from census fixtures incl. anti-blob (T2); sidecar tokens realize the only real causal seam found (T2, honest ~0-at-birth stated); link identity + twice-run byte-identical (T1/T3); instrument-only weave + universal-joinability promotion + arrival-order (T4); cites-every-bead (T5); derived-follows-at-read (T6); A11 zero-mutation shadow (T7); per-layer flags byte-identical throughout; one-merge staged-wake preserved (T8 stop).
