...
- [ ] **Step 2 — implement:** emit `consolidation_telemetry` from raw→daily (`consolidate_daily`), the dream cycle, and the reflection hook. `rails_blocked` = untrusted-filter drops (consolidation) / uncited-reflection drops (reflection). **Model-label fix — STRICT (truth-in-reporting only, NOT routing):** **Do NOT change routing or the served model.** The `llamacpp` backend already ignores the requested `"gemma4:26b"` and serves Qwen (`LLAMACPP_MODEL` / `model_alias='qwen36-27b'`) — leave that exactly as-is, byte-equivalent. The ONLY change is the **telemetry's `model=` field**: report the **actually-served** model (`PRIMARY_MODEL` / the served alias from `/props`), NOT the misleading requested label. This makes any future model drift visible without touching which model runs. Optionally the dead `MODEL = "gemma4:26b"` *requested-label constant* may be annotated as ignored-under-llamacpp, but **no behavior except telemetry may change.**
