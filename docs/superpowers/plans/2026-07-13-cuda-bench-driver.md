...
- [ ] **Step 0: Pre-code witness (bootstrap anchor).** BEFORE writing any
  code, run the raw suite command once in the fresh worktree and save its
  failure lines + collection count to a scratch note (not the authority —
  just `pytest tests/ -q 2>&1 | tail -20` plus `--collect-only -q | tail -1`
  into the task log). This is the clean-base state the first commit is
  measured against.
...
- [ ] **Step 1: Write the failing tests FIRST** (`tests/test_bench_baseline.py`,
  ...
  a parameterized failure id containing spaces
  (`FAILED tests/t.py::test_x[a b] - boom`) parsed back INTACT; second
    ...
    for line in output.splitlines():
        for prefix in ("FAILED ", "ERROR "):
            if line.startswith(prefix):
                # Full node id, NOT line.split()[1]: parameterized ids may
                # contain spaces ("test_x[a b]"); pytest -q separates the
                # id from the message with " - ", so split there, once.
...
def _validate_shapes(base: dict) -> None:
    ok = (
        ...
        and (base["pytest_status"] == 1) == bool(base["failures"])   # status↔list consistency
...
- [ ] **Step 6: Bootstrap equality check, then commit.** Compare the
  recorded authority's failure list against Step 0's pre-code witness:
  the sets must be IDENTICAL except for additions in
  `tests/test_bench_baseline.py` itself being green (i.e., the helper
  introduced zero new reds anywhere else — the first commit cannot bless
  its own regressions), and the collection count must be ≥ the Step 0
  count. Only then commit
