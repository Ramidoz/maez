...
**Interfaces:**
- Produces (exact):
  - `def systemctl_command(subcommand: str, unit: str) -> list[str]` — user-scope whitelist `{"show", "is-active"}` only; anything else raises `ValueError("mutating_systemctl_forbidden")`. Containment has one separate hard-coded system-scope `show maez.service` builder; both builders share the single frozen executable literal and neither can construct a mutating verb.
