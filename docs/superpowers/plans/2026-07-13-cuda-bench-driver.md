...
#   "cuda_bench_driver.phase_packet.v3": _decode_phase_packet
...
- Produces (exact):
  ...
  - **B5 sealed-memfd GREEN amendment (ruling 2026-07-15):** the launcher creates one `MFD_EXEC|MFD_ALLOW_SEALING` snapshot, writes the entry bytes, applies WRITE/GROW/SHRINK/SEAL seals, hashes only after sealing, and executes that same object. Both inode replacement and in-place overwrite REDs must still execute the pre-mutation sentinel. Host no-exec policy is a typed `spawn_failure`. The pin scope is explicitly the entry executable only; the runtime manifest separately owns every dynamically loaded backend library. `phase_packet.v2` (replacing v1; canon stays 22) persists `pinned_path` + `pinned_sha256`, rejects v1, and binds/joins them to the phase-specific executable authorities. (Historical wording preserved: v2 replaced v1 here; the 2026-08-03 residual amendment later replaced v2 with `phase_packet.v3`, which additionally persists the recomputed per-cycle unload residuals.)
  - **B5 manifest-bound cwd amendment (ruling 2026-08-01):** retain the sealed memfd unchanged. For `SpawnPin.kind == "binary"`, derive the backend working directory only as `pin.pinned_path.parent`; never accept a caller cwd or second path constant. Fresh static observation creates an in-memory `ReleaseDirectoryProof(manifest_sha256, directory_dev, directory_ino, snapshot_sha256)` from the SAME held directory fd that validated the phase manifest. Minting is bracketed for BOTH Vulkan and CUDA: validate manifest/named files, compute the full snapshot, then revalidate manifest/names/named files/full snapshot before returning; any in-bracket mutation refuses. The common snapshot canon covers every top-level regular file by name/bytes and every literal symlink by name/target; other entry types refuse. `RealServerLauncher` reopens only the derived parent through a per-component `O_NOFOLLOW` directory walk, matches `(dev, ino)` and the full snapshot, and packages the fd with the exact `SpawnPin` object and proof in a private guarded launcher capability. `spawn_pinned` accepts no raw cwd/release fd and refuses binary launch without that capability or with a capability bound to an equal-but-distinct pin; only the inert guard receives the raw fd and calls `fchdir` after the go byte and before fd-exec. `_validate_production_execution_contract` additionally joins `proof.manifest_sha256` to `static.checks["library_manifest"]` for Vulkan or `runtime_identity.runtime_manifest_sha256` for CUDA. Directory-open, proof/manifest join, or `fchdir` failure is typed `spawn_failure` with no target execution. The common phase-tail path recomputes the full snapshot before a completed packet may publish; pre-admission binary failures also recheck and close the held fd without replacing an existing `_BinarySpawnFailure` carrier or weakening cleanup/pid-reuse/interruption precedence, and the normal handoff retires the private capture before returning. Release drift is used only when it is the first tail refusal and never replaces an existing containment, kernel, or journal failure. Any new/changed/deleted entry is terminal and unscoreable. `python_file` rehearsal receives no cwd fd and remains byte-identical. RED-first sequence: (1) compile a real ELF loader plus sidecar and witness sealed-memfd/wrong-cwd failure; (2) witness sealed-memfd/manifest-bound-cwd success; (3) prove cwd follows a changed pin parent with no independent authority and prove direct binary spawn cannot omit or forge the launcher capability; (4) reject right-shaped but wrong-manifest directories, component/final symlinks, identity/snapshot drift, and open/`fchdir` failures before the target sentinel; (5) mutate/create/delete release entries and prove the actual phase publication path mints no completed packet; (6) rerun the existing memfd mutation, guard/pidfd, diagnostic capture, finalizer/residue, production packet, and rehearsal-cwd suites unchanged. The real b9596 source basis is `ggml/src/ggml-backend-reg.cpp`: default search paths are executable directory followed by `std::filesystem::current_path()`.
...
- Produces (exact):
  ...
  - `stream_completion(port: int, prompt: str, *, clock: Clock, opener=None) -> TurnMeasurement` — POST `/completion` on `127.0.0.1` with body `{"n_predict": FROZEN_TURN_N_PREDICT, "prompt": prompt, "stream": true}` (streaming MUST be requested explicitly — without `"stream": true` llama-server returns one non-streamed body and TTFT is unmeasurable; `n_predict` MUST be present — without it llama-server generates unboundedly and phase viability depends on the model choosing to stop, witnessed in window ab-20260802-2149 where a healthy control at ~70 tok/s was cancelled by `REQUEST_TIMEOUT_MS` past 1,675 tokens. One frozen value for every warmup and measured turn on both backends keeps the A/B fair. Amended 2026-08-02 with the review-gate ruling trail.), streams SSE with total read ≤ `RESPONSE_BYTE_CAP` (`response_too_large`), per-turn wall clock ≤ `REQUEST_TIMEOUT_MS` (`http_timeout`); `TurnMeasurement(ttft_ms: float, e2e_ms: float, content: str, timings: dict, terminal: dict)`; TTFT = first `data:` event whose JSON has non-empty `content`; opener param is the urllib opener seam (rehearsal passes a no-proxy, no-redirect opener; both tiers use `urllib.request.build_opener()` with `ProxyHandler({})` and a redirect-refusing handler).
