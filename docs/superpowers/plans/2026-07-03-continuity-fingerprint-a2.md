...
**Architecture:** A `core/continuity/` package: `envelope.py` (resolve the live frame mode + component-hash snapshot), `probes.py` (the open battery + wording-audit rationale), `store.py` (A2-private sqlite), `sampler.py` (out-of-band run: envelope → battery → embed → store, zero writeback), `meter.py` (drift metric + robust aggregation + brain_swap correlation + confound/insufficient/era rails). Surface: `scripts/continuity_fingerprint.py` behind `MAEZ_CONTINUITY_FINGERPRINT`. Embedder is the existing `memory.embedder` MiniLM instrument.
...
**Files:** Create `core/continuity/__init__.py`, `core/continuity/envelope.py`; Test `tests/test_continuity_envelope.py`
...
class EnvelopeModeTests(unittest.TestCase):
    def test_self_card_mode_uses_assembled_card_without_time_line(self):
        from core.continuity import envelope
        with mock.patch("core.continuity.envelope._self_card_enabled", return_value=True):
    ...
    def test_legacy_mode_uses_voice_card_text(self):
        ...
        with mock.patch("core.continuity.envelope._self_card_enabled", return_value=False):
            env, snap = envelope.build_probe_envelope()
    ...
    def test_never_calls_voice_card(self):
        ...
        import inspect, core.continuity.envelope as e
...
**Files:** Create `core/continuity/probes.py`; Test `tests/test_continuity_probes.py`
...
class BatteryTests(unittest.TestCase):
    def test_battery_is_small_stable_and_versioned(self):
        from core.continuity.probes import BATTERY, BATTERY_VERSION
    ...
    def test_every_question_has_a_wording_audit_rationale(self):
        from core.continuity.probes import BATTERY
        for q in BATTERY:
...
**Files:** Create `core/continuity/store.py`; Test `tests/test_continuity_store.py`

- [ ] **Step 1: Failing tests** — a fresh tmp store round-trips a run (snapshot + per-question answer text + distances); `list_runs()` returns them ordered; the store path is A2-private (never a shared DB name). No vectors stored (assert no BLOB/vector column).
- [ ] **Step 2-4: RED → implement** `ContinuityStore(path)` sqlite: table `probe_runs(run_id, ts, era, self_card_applied, base_model, soul_base_hash, soul_local_hash, frame_text_hash, policy_hash, embedder_id, battery_version)` + table `probe_answers(run_id, question_id, answer_text, dist_short, dist_mid, dist_long)`. `record_run(...)`, `list_runs()`, `answers_for(run_id)`. **No vector column.** — [ ] **Step 5: Commit** `feat(continuity): A2-private probe store (answer text + distances, no vectors)`

---

## Task 4: `sampler.py` — out-of-band run + terminal-sink

**Files:** Create `core/continuity/sampler.py`; Test `tests/test_continuity_sampler.py`
...
**Files:** Create `core/continuity/meter.py`; Test `tests/test_continuity_meter.py`
...
class MeterTests(unittest.TestCase):
    def test_drift_is_cosine_distance_per_question_then_median(self):
        ...
        from core.continuity.meter import aggregate_drift
...
```
- [ ] **Step 2-4: RED → implement.** `aggregate_drift(distances)` = median (or trimmed mean). `verdict_for_swap(runs, swap_ts)`: group runs into the era they belong to; require ≥ `MIN_SAMPLES` within-brain before AND after → else `insufficient_data`; if any of {soul_base_hash, soul_local_hash, self_card_applied, policy_hash, era} changed across the boundary → `confounded`; else compare cross-swap jump vs within-brain drift baseline → `continuity_survived` / `discontinuity` with the ratio. Anchors: short/mid/long as "last K runs" (K constants named + justified, not wall-clock). — [ ] **Step 5: Commit** `feat(continuity): law-2 meter — median drift, confound/insufficient/era rails`
