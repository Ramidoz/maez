...
## v3.1 mechanical contract additions (authoritative deltas over the task bodies — each is a REQUIRED RED test)

These close caller-can-smuggle-trust pockets. Where a v3.1 contract conflicts with a task body below, **v3.1 wins.**

- **Task 1 (egress):** add RED tests — (a) `getaddrinfo` results **post-filtered** to loopback sockaddrs even for an allowed port (assert every returned `sockaddr[0]` is loopback); (b) `_addr_is_allowed` handles **IPv6 4-tuples** (`(host, port, flowinfo, scopeid)`) and AF_INET6 sockets; (c) **phase-scoped** test: with `allow_loopback_ports=(VARIANT_PORT,)` the JUDGE_PORT is blocked, and vice-versa — the two are **never open together**; (d) **restoration** test: after `with no_egress(...)` exits, all five APIs (`create_connection`/`connect`/`connect_ex`/`sendto`/`getaddrinfo`) are the original objects again.
- **Task 2 (config):** add RED tests — (a) **fail-closed**: empty/missing variant config raises `VariantConfigError` (no silent empty run); (b) **no `model_config` fallback**: loader never reads `MAEZ_PRIMARY_MODEL` (assert it raises rather than defaulting); (c) **source+hash**: `load_variants` returns/records a config `source` + content `hash` (provenance, like `fixture_manifest_hash`); (d) **pathless**: `validate_endpoint` rejects any URL with a path (`http://127.0.0.1:11434/api/chat` → reject); code appends `/api/chat`, config supplies host:port only.
- **Task 3 (real seam):** add RED tests — (a) adapter sends **`think=False`** to the backend; (b) **response-shape assertion** (a malformed backend response → `FailCode.BAD_SHAPE`, not a crash); (c) **no raw exception escapes** the adapter (a `requests.ConnectionError` → `FailCode.REFUSED`, caught, not propagated to `focused_synthesize`); (d) **payload override** test: `options={"num_predict":256}` overrides `variant.chat_kwargs={"num_predict":999}` (caller wins), other variant kwargs preserved.
- **Task 4 (judge):** add — `BlindAnswer`/`PairwiseInput` carry a **`sample_id`** so the k repetitions of one probe are grouped (not treated as distinct items); a test asserts grouping by `sample_id`.
- **Task 5 (gates):** **`voice_lint(answer_text) -> VoiceLintResult(ok: bool, reasons: tuple[str,...])`** is a **deterministic substrate function** (closed reason codes: `too_long`/`too_short`/`cognition_verb`/`gendered`), tested on real strings. `hard_gate_fail_reasons` consumes the **computed** result, never a caller `voice_lint_ok` bool. Sort key is **ops-before-speed** (fixed). Add a test that the hard-gate function signature has **no parameter** that could carry a judge win-rate (advisory never enters hard gates).
- **Task 6 (packet):** **non-vacuous** sentinel test — construct a path that *attempts* to place `FABRICATED_SENTINEL` into a serialized field and assert `__post_init__` rejects it OR it's absent from the JSON (the test must be able to FAIL if the boundary leaks). `OpsRubric` fields are **closed enums** (`ApiFamily`, `Topology`, `GpuContention`, `StartupHealth`, `RestartRecovery` enums; bools for the rest) — not free strings. `BenchPacket.__post_init__` **validates `screen_result` is a `ScreenResult`**. The forbidden-content check matches **compound/substring names** (`answer_text`, `raw_reply`, `evidence_snippet`), not just exact field names.
- **Task 7 (orchestration):** the **written** debug-dump file is opened in a test and asserted to contain `{"provenance":"UNTRUSTED","quarantined":true}` (prove the metadata is in the file, not just returned by a function). The judging phase opens **only** the judge port (variant ports closed) — assert via the phase-scoped guard.
- **Wording:** no "verdict"/"recommend" for the packet anywhere — "result"/"report" only (the judge's `A/B/TIE/INVALID` "verdict" is fine; that's the judge, not the packet).
