...
**Tech Stack:** Python 3.10; host tests `/home/rohit/maez/.venv/bin/python -B -m unittest` (NOT pytest); device adds `onnxruntime` (CPU ONNX reference), `cuda-python<13` (TRT buffer binding compatible with Jetson CUDA 12.x), `numpy<2` (keeps apt OpenCV on the NumPy 1.x ABI), `cv2` (V4L2), TensorRT 10.3 + `trtexec`. Reference ONNX detect/decode via the InsightFace SCRFD/ArcFace models.
...
2. **Deps:** one-time owner step if needed: `sudo apt install python3-pip`, then `bash devices/jetson_presence/setup_models.sh deps` → installs `onnxruntime`, `cuda-python<13`, and `numpy<2` (so the CUDA binding matches Jetson CUDA 12.x and apt OpenCV still imports). (No `lock-hashes` needed — the manifest already ships real pinned hashes; `lock-hashes` is only for swapping the pack later.)
