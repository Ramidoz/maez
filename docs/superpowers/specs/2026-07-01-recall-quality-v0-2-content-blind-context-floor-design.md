...
3. **v0.1 treatment code is removed, not just dark.** The build deletes the kind-branch in the floor predicate, the kind-aware fallback preference, and the type-weight application in the floor path. `_recall_candidate_kind` is **kept** — telemetry/witness only; a structural guard asserts no *decision* path reads it.
...
**Structural guards:** no decision path reads `_recall_candidate_kind` (AST/probe-proven, like the b1a guards); v0.1 treatment code absent; new flags owned only by `memory_manager.py`; dream/soul confinement unchanged.
