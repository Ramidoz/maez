...
### Receipt-grade is currently ASPIRATIONAL for the rail catches (Codex HOLD fix #1)
Verified: judge fabrication writes return **no id**, action-claim mismatches emit **only a log line**, redo `accepted|floor` outcomes are **logger-only**. So Task 0/build must first give each rail catch a **durable content-light receipt id** (return the fabrication row id; write a small redo-outcome receipt row; capture the mismatch receipt) — **or the class is EXCLUDED from v0 until it has one.** "Receipt-grade or nothing" is enforced by test: a scar write without a resolvable receipt ref must FAIL validation, not degrade to a log-line citation.

### Landing 1 — consequence row (the planner keeps its hints, scars get neutral rendering)
Every scar-class event writes a `consequence_memory` row (existing API; **`card_rejected` WIDENS the existing write at decision_pipeline ~2117 — capture the currently-discarded row id, never a second row for the same rejection**; test: exactly one consequence row + one scar episode per rejection). The `feedback` field for scar classes is **restatement-only**: derived verbatim from the rejection reason / audit flag / correction content — never synthesized advice.
**The prompt-block conflict (Codex HOLD fix #2):** the existing planner renderer is literally `[LEARNED FROM PAST MISTAKES]` injected into planning prompts — a standing past-failures section, exactly what pin #2 forbids. Fix: **scar classes are EXCLUDED from that block**; if scar rows should reach the planner at all, they render through a separate neutral formatter (event + correction + receipt, no header editorializing) — and a test asserts scar rendering contains no shame/directive vocabulary (`mistake`, `never`, `should not`, `avoid`, `failed to` as instruction). The existing block keeps serving `tool_failure` unchanged (out of scope to reword it).
...
2. `memory_voice` enum semantics → pin the scar value (reuse vs add; write-safety like TrustTier — the A3 lesson: check the *writer* validation, not just readers). **Plan-watch (Codex): `memory_voice="maez_self"` is classified and damped in the parked promotion path** — Task 0 must prove scar episodes remain ordinary recall candidates under whatever voice is chosen (or surface the trade to Rohit if every candidate value carries baggage).
