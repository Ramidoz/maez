...
> **Nothing durable may enter Maez's memory that cannot name the lived rows it came from.**

This is the reflection rail's citation-lock and A4's "receipts prove the tie or there is no tie" — promoted from per-organ defenses to the memory constitution.
...
- Adopt the law for **new** organs: A1 scars, A8 annotations, A4 narrative already comply by design (all citation-locked). New durable stores cite turn_ids or receipt-store ids from day one.
- Build the **evidence-envelope builder** — declared but unbuilt (`envelope_schema.py:22-26`); without it, post-birth rows carry thin envelopes and the food is poor. This is the highest-leverage pre-birth engineering item in the body.
...
2. **The envelope builder is unbuilt.** Until it exists, rows are thin and the digest is thin. That's why it's Phase 0 — and why this proposal creates pre-birth work rather than waiting for birth.
...
4. **Scope gravity.** A12 touches every organ's feed *eventually*. The law prevents this from becoming a big-bang only if Phase 4 stays one-slice-per-organ with cooling-off. If any phase starts bundling, stop and re-read the coherence-organism NO-GO.
...
2. **Green-light the envelope builder as pre-birth work** (Phase 0b) — engineering, no covenant surface.
