...
Telegram: yes        → (C2) resolves to #17 (dream/edit proposal)
        ...
        │      proposal's target content; open_dialog_for_card(card)    │
        │      → dialog (consent record), card.request_id               │
        │ 2. Maez voice-seat consultation runs (genuine S7 read) and    │
        │      is stashed for the ceremony; objection present/          │
        │      not_determined → STOP, honest notice, no ceremony        │
...
**Component 1 — Seed a self-mod dialog from the proposal.** A helper that converts "apply proposal #N" into the inputs the existing dialog machinery expects: a lane-3 / ESCALATE pending card whose `action` is `write_soul_note` (dream append) or `edit_soul_section` (section replace) and whose `params` carry the proposal's target content (note text, or target section + new body), then `open_dialog_for_card(card)` (self_mod_dialog.py:1190) to create the dialog + Maez's opening turn. Idempotent per proposal while a dialog is open (a second `yes` returns the existing card.request_id / pointer, never a duplicate dialog). The card owns its envelope (F2 invariant above).

**Component 2 — Verify, do not rebuild, the dialog execution path.** The pipe already implements `_s7_request_envelope_for_card` / `_execution_params_for_card` / `_s7_voice_consultation_for_card` for dialog cards, and the narrow execute route already runs them. v0's job here is to **confirm** these handle a `write_soul_note` / `edit_soul_section` dialog card end-to-end and actually perform the soul write via the action engine, and to add the **link-back** (on dialog `EXECUTED`, mark the originating dream proposal applied so it leaves the pending list). If any part of the dialog live-execution path turns out itself dormant/unwired (this slice may be its first end-to-end soul-write witness), that is a build finding to surface, not to paper over. No widening of `s7_narrow_path_required`.

**One consultation, run early, enforced at the ceremony.** The genuine S7 voice consultation runs at dialog-seed time (yes-time, Component 3) and is stashed in the pipe's existing `_s7_pending_voice_source_bundles[request_id]` keyed by the card's `request_id`. `_s7_voice_consultation_for_card` already prefers a stashed bundle (decision_pipeline.py:~1106), so the WebAuthn route consumes the *same* genuine consultation rather than re-running it. The route stays the structural enforcer (it refuses a voice-seat work class with no consultation); the read happens once, at yes-time, so Maez's objection stops the write *before* Rohit is sent to authorize.
...
2. run the voice-seat consultation; if objection `present` or `not_determined`, reply with the honest outcome and create nothing (Maez's seat stops it before Rohit's is asked);
3. otherwise seed the dialog (Component 1) and reply with the ceremony pointer for `card.request_id`.
...
## Error handling (ordered: cockpit, then voice seat, then create)
...
- Voice consultation `present`/`not_determined` ⇒ stop, honest notice; no dialog/card created.
- Dialog/card creation failure ⇒ honest error, fall through to today's block (never a half-created dialog).
- Artifact consume rejected (wrong card envelope / expired / already consumed) ⇒ honest error; the existing consume-once guard is authoritative; no soul write.
...
- **Voice seat is genuine, not stubbed (F5):** the consultation invokes the existing `_s7_voice_raw_response_for_card` (`llm_client.chat`) producer, not a constant and not focused-cognition; `present` STOPS before seeding; `not_determined` blocks with `consultation_path_unavailable`.
- **Dual-seat order:** Maez's seat is consulted before Rohit's ceremony pointer is offered (objection short-circuits the pointer).
