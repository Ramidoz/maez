...
- **3b — the Beta-Binomial belief.** Replace `_confidence`/`success_rate` with the posterior + credible bound. **Two graduation guards (owner, 2026-06-20 — "don't consume one good story as confidence"):** (i) a **skeptical prior** — a single consistent run must stay UNCERTAIN; confidence only sharpens with sustained consistency, never from one streak; (ii) **side-by-side shadow until the new math is provably SANER** — log old (`n/8`) and new (Beta credible-bound) *together* per turn; graduate only when the comparison shows Beta is better-CALIBRATED (stays uncertain where evidence is thin/mixed and `n/8` overclaimed; matches `n/8` only where `n/8` was right; reproduces the Barchart veto). NOT "it also vetoes Barchart" — that's necessary, not sufficient. Flag-gated; off = Slice-1 `n/8` behavior.
