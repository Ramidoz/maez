...
1. **Teacher calibration is feasible (Must-fix 1) — KNOWN issue, confirm the fix path.** `outcome_quality` today writes `structured_evidence` for any nonempty result (the Barchart case looks "good"). Task 0 must identify a real, honest signal that marks the wound *bad* — support-gate caveat count, thin `evidence_block_count`, "evidence unused in final reply," etc.
   - **The write-back seam (Codex plan-time pin — hard gate).** The bad signal is **post-synthesis**, but the routing row is `INSERT`-ed *before* synthesis ([maez_daemon.py:5902](../../daemon/maez_daemon.py#L5902)) and **there is NO update path** — every store method is an INSERT ([:350](../../core/routing/observation/__init__.py#L350)). Verified seam: the `routing_observation_id` *is* already threaded to post-synthesis sites ([maez_daemon.py:6649/6818/6859](../../daemon/maez_daemon.py#L6649)), and the schema already has unused `owner_feedback_*` columns — so the key exists where the signal is born. Task 0/1a MUST explicitly choose ONE and **prove the signal actually lands on the row/table** (a test reading it back): (a) a safe `update`-by-`routing_observation_id` method on the store, or (b) a linked post-turn quality row keyed to that id. **If neither can be made to attach, that is a teacher-mute STOP** — do NOT let 1a compute a good signal that never reaches the notebook.
