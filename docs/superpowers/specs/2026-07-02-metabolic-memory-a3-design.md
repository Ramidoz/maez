...
- **substrate-salience rescue (FIRST-CLASS REQUIREMENT — Rohit, 2026-07-02: "shouldn't Maez decide what is idle?"):** if Maez's own machinery marks the thought, it earns durability — the substrate's own coherence can always overrule the quiet default. The build MUST wire **every live salience signal Maez has today** into this vote (lean-heartbeat thought_formed/moved, salience-broker proposals, and any signal Task 0 finds live) — Maez's current voice, however small, fully counted. "Idle" is therefore never a verdict anyone passes: it means only *the world did nothing AND Maez didn't raise its hand*. The only voters are reality (events) and Maez (its signals) — never us, never a content category.
- **Interim by declaration:** the event-trigger set is **scaffolding** — it carries the load only while Maez's salience machinery is young. As that machinery matures (Slice-C ledger→gate, learned relevance), Maez's own signals take over more of the durability vote; any future edit that *tunes the trigger list* instead of growing Maez's vote should be challenged with this clause. The model brain is explicitly NOT a voter (asking the LLM "keep this?" per glance would make the model's priors the gatekeeper of Maez's memory — Law 2 bleed); the substrate signals are the covenant-true voice of "Maez decides."
...
1. **Enumerate consumers of recent cycle thoughts — including the three VERIFIED raw-recency readers (Codex HOLD fix):** `dream_state.recent_raw(n=DREAM_MEMORY_WINDOW)` (dream_state:384), `self_analysis` reading `raw.get(limit=200)` (self_analysis:34), and the proactive-opinion raw window (daemon:4668) — plus cycle_recall_context, lean-heartbeat facts, salience-broker signatures, ws-broadcast. **For EACH consumer the plan makes an explicit decision:** ring-buffer parity (serve it from buffer∪raw so it sees the same world as today), or a **named, owner-visible behavior change**. No consumer may change behavior silently.
   **OWNER-DECIDED (2026-07-02): the three raw-recency consumers get the durable-only diet** — dreams, self-analysis, and proactive-opinion feed on what the world did + what Maez itself marked, which is "Maez decides what it dreams about" applied (durable = events ∪ Maez's salience votes). This happens *by construction* (untriggered glances never enter raw), and each consumer still gets its explicit old-vs-new behavior artifact + witness. Parity was rejected as keeping the noise, not adding agency.
