...
- **Three new numbers** added to `GroundednessVerdict`: `reply_grounding` (float), `grounded_sentences` (int), `total_sentences` (int).
- **Emitted** alongside `citation_coverage` in the `recall_outcome` log line ([maez_daemon.py:1336](../../daemon/maez_daemon.py#L1336)) and added as **three numeric columns** to the `focused_cognition_runs` store ([focused_cognition.py:1520+](../../core/routing/focused_cognition.py#L1520)) via an **idempotent migration** (ALTER TABLE ADD COLUMN guarded so re-runs are safe). **Numbers only — the reply text is NEVER persisted** (the store is content-light by construction; preserve that).
...
## Scope
**IN:** the `reply_grounding`/`grounded_sentences`/`total_sentences` computation in `check_groundedness`; a small local sentence-splitter; the new `GroundednessVerdict` fields; the `recall_outcome` log addition; the focused-store 3-column idempotent migration + record(); tests. **OUT / NEVER:** changing `citation_coverage`'s formula or `verdict.verdict`; any behavior/voice/fallback gate; persisting reply text; claim-vs-self-expression classification (deferred to a possible v1); touching recall, synthesis, or the anchor (those are slices 2/3).

## Make-or-break / guards (review)
1. **Zero behavior change** — diff touches only metric computation + logging + store columns; no reply path, no `verdict.verdict`, no fallback logic altered. A test asserts the reply text is byte-identical with and without the new metric.
2. **Content-light** — assert the store/log carry only numbers, never reply/sentence text.
3. **Metric correctness** — unit tests: a reply where every sentence cites a valid `[E#]` → 1.0; a 16-item set with a 2-sentence reply both validly cited → 1.0 (NOT 0.125, proving the denominator fix); an all-uncited self-narrative → 0.0; a sentence citing an invalid `E99` → not grounded (and still flagged unmatched by the existing path).
4. **Migration idempotent** — adding the columns twice (re-open) does not error; old rows read with NULL/0 defaults.
5. **Deterministic** — same reply+working_set → same numbers (no model call).
