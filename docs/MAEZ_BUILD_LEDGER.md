...
| Voice Boundary v0 | LIVE_WITNESSED (A/B); C pending | `capability_prompt_block()` shared by daemon ambient + focused cognition; Telegram `_handle_command` C1 for proposal slash commands | n/a | `MAEZ_VOICE_BOUNDARY_ENABLED` | A/B PASS on :11435 brain bench (W1+W2 healed); Telegram C pending | merge+flag+restart DONE; Telegram C witness | JSON envelope quotable but did not manifest on witness turns | owner Telegram witness for /proposals,/show,yes | 0b15e12 | 2026-06-12 | claude |
