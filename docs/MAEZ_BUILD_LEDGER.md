...
| S7 ceremony bridge v0 | PLANNED_PLAN (0a NO-GO) | spec @7700ce5 / plan @c66e56e | self-mod-dialog soul-write path is DORMANT | `MAEZ_S7_CEREMONY_BRIDGE_ENABLED` | 0a proof FAIL (verified): `build_work_request_envelope` raises `guarded S7 work requires derived_aggregation_group` | none yet | blocked on Task 0.5 below; do NOT wire ceremony on the dormant leg | Task 0.5 first, then re-run 0a, then bridge Task 1+ | 0a948f2 | 2026-06-12 | claude (verified codex 0a) |
| S7 soul affected-ref derivation (Task 0.5) | PLANNED_SPEC | `operator_user_boundary.derive_affected_refs` (:825) needs a `write_soul_note`/`edit_soul_section` branch -> `file:`+`paths.soul_combined_path()` | `derive_work_class` returns self_modification but `derive_affected_refs` returns () for these actions (asymmetry) | n/a | root cause verified on main | none | precursor to the bridge; both soul actions must derive the SAME soul ref (shared aggregation group — confirm covenant semantics); no executable param leak | small one-branch fix + same-ref aggregation test, then re-run 0a | 115142c | 2026-06-12 | claude |
