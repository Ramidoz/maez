...
| Voice Boundary v0 | BUILT_ASLEEP | `capability_prompt_block()` shared by daemon ambient + focused cognition; Telegram `_handle_command` C1 for proposal slash commands | n/a | `MAEZ_VOICE_BOUNDARY_ENABLED` | gate tests, awaiting A/B + Telegram witness | merge+flag+restart+witness | JSON envelope may still be quoted; witness decides voice effect | Claude covenant review, then owner witness | be9b505 | 2026-06-12 | codex |
| Capability card render form | BUILT_ASLEEP | `core.cognition.capability_card._build_capability_envelope` under strict voice flag | old prose path preserved flag-off | `MAEZ_VOICE_BOUNDARY_ENABLED` + `MAEZ_EVIDENCE_PRECEDENCE_ENABLED` | envelope tests; no dashboard jargon guard | merge+flag+restart+witness | rendered-form only; probes unchanged | A/B webapp/HTTP brain-bench witness | be9b505 | 2026-06-12 | codex |
| Command surface on Surface V2 (`/proposals`, `/show`) | BUILT_ASLEEP | `telegram_adapter._handle_command::_try_command_proposal_surface` before LLM fallthrough | generic slash fallthrough to brain | `MAEZ_VOICE_BOUNDARY_ENABLED` | command tests; no-brain fake; source-order guard | merge+flag+restart+witness | reuses MaezMessageHandler accessors + shared last-shown; no new engine | Telegram witness: `/proposals`, `/show`, natural `yes` | be9b505 | 2026-06-12 | codex |
