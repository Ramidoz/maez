...
> - **wants.db — 5/5 rows `explicit_api` + `{"seed": true}`:** witness scaffolding from the Valence v0.2 / proprioception organ witnesses ("confirm the thermometer reads a satisfied want"), satisfied via `owner_confirmed`. No drive-originated want exists (consistent: `drive_curiosity` unregistered — there is no autonomous author to have written one).
...
>
> **Sharpened gate definition (replaces "0 self-authored wants/thoughts"):** *the self-formation loop is dormant iff no row in wants/wonderings/private-thoughts/soul-proposal stores carries provenance of autonomous self-directed authorship (drive-originated want, self-initiated wondering, self-directed soul/thought proposal). Non-authored provenance classes — seeded/witness scaffolding, smoke/self-test/reseed/manual, want-derived, `reasoning_residue`, `crisis_signal_held` — do not violate the gate.* Verified against that definition: **dormant. Blocker #2 CLOSED.** Disposition of the piles themselves (gestation-stamp, archive, deweight) folds into the A7 decision and the birth-ceremony era-stamping section — classification here only establishes they are not self-authorship.
