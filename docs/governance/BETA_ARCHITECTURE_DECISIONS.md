...
**Why now.** Slice 2 canonicalized at `f0d14e3`, preserved review history at `fb76a13`, and landed implementation across `ba4a545` through `eb611e9`. The implementation reused existing organs (`wonderings`, `wondering_cycle`, `wondering_pursuit`, `subjective_duration`, `temperament`) rather than inventing a parallel substrate.

**2026-07-06 runtime correction.** The prior "live-witnessed in the second crossing" wording is withdrawn as stale. Whole-body audit re-checks found `drive_driven_curiosity.register_default_encounter_producers()` has no production caller, so the felt-organ is **canonical and implemented, but runtime-unwired**. Wiring it is a birth/provenance-wall decision, not a mechanical cleanup.

**Consequences.**
- V1 encounter producers exist for `WONDERING_GENERATED`, `EXPLICIT_OWNER_FLAG`, and `SUBJECTIVE_DURATION_MEANINGFUL_EVENT`; the last path is recursion-gated and deduped. They are not registered in production as of 2026-07-06.
