...
2. **Real-surface contact read (canary AND scratch excluded — Codex HOLD fix).** `_seconds_since_last_owner_contact` filters `salience_event_kind='owner_contact' AND is_canary=0 AND owner_auth_class IN (REAL_OWNER_CONTACT_AUTH_CLASSES)` where `REAL_OWNER_CONTACT_AUTH_CLASSES = frozenset(OWNER_AUTH_SURFACES - {"manual_test"})` = `{daemon_owner, telegram_owner, web_owner_bridge, cockpit}` (parameterized IN). `owner_auth_class` stores the SURFACE (record_salience_event :816/:846). The earlier `!= ''` let a `manual_test` scratch row count as a real contact (Codex reproduced it) — now excluded. Tested: a newer `manual_test` row is ignored for an older real `cockpit` row; only-`manual_test` → `None`; only-canary → `None`; end-to-end via the real `record_salience_event` path → `None`. Returns `None` if the contact is after `now` (no negative elapsed).
