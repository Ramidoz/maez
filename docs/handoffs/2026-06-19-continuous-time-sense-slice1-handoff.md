...
**Date:** 2026-06-19. **Branch:** `continuous-time-sense-slice1` (tip = the latest commit on the branch; see `git log`. local-only, NOT pushed, NOT merged).
...
## Commits (8 — see `git log main..HEAD` for the live list)
...
- (handoff) docs: this review-gate handoff.
- (read-only fix) fix: `peek()` is truly read-only — the clock-degraded write moved to `current()` (**Codex HOLD fix — see below**).

Net vs main: `subjective_duration.py +`, `maez_daemon.py +35`, tests, 1 proof doc. Surgical.
...
3. **`peek()` is TRULY read-only; the degraded write lives in `current()` (Codex HOLD fix).** `_compute()` is now PURE — it writes nothing and returns `(snapshot, degraded_latest)`. `peek()` ignores the degraded signal → writes nothing, EVEN on the clock-degraded path (the original bug: `_compute` recorded a `clock_degraded_event`, so a heartbeat `peek()` in a clock-skew window wrote salience events — Codex proved 0→1). `current()` owns the degraded write: on `now < latest ts` it records the event exactly once + returns the snapshot with NO sample insert (honesty preserved); on the normal path it inserts (byte-preserved except the additive `compute_version`). Tests: `test_peek_does_not_write_on_clock_degraded` (peek leaves salience + sample counts unchanged on a backward clock) + `test_current_still_records_clock_degraded` (current records it once).
