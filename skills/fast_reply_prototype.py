...
# (skills.calendar_perception is allowed only inside the calendar worker,
# not on the reply hot path — it's not in this list because the worker
# does the import lazily and the hot path never triggers it.)
...
DEGRADED_REPLY_TEXT = (
    "I'm here, but my drafting attempt came back empty. "
        ...
        if retry_result.success and fast_backend_local.is_visible_reply(retry_result.text):
            ...
            if cloud_eligible:
                ...
                if isinstance(rt2, dict):
                    ...
                    metrics.cloud_redacted    = metrics.cloud_redacted or bool(rt2.get('changed', False))
                    metrics.cloud_redactions  = metrics.cloud_redactions + int(rt2.get('total_redactions', 0) or 0)
                    ...
                    for k, v in (rt2.get('internal_counts') or {}).items():
                        metrics.cloud_redaction_internal[k] = metrics.cloud_redaction_internal.get(k, 0) + v
