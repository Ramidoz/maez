...
class PresenceSnapshot:
    ...
    def format_for_context(self) -> str:
        ...
        if self.person_identified == "stranger" and self.rohit_present:
            return "[PRESENCE] Someone is at the desk — not the owner."

        if self.just_arrived:
            if self.person_identified == "the owner":
                return "[PRESENCE] the owner just sat down at his desk."
            return "[PRESENCE] Someone just sat down at the desk."

        if self.just_left:
            return "[PRESENCE] the owner just stepped away from his desk."

        if self.rohit_present:
            who = self.person_identified if self.person_identified != "unknown" else "the owner"
            ...
            if self.absent_minutes < 2:
                ...
                return f"[PRESENCE] the owner away — {int(self.absent_minutes)} minutes."
            else:
                return f"[PRESENCE] the owner has been away {int(self.absent_minutes)} minutes."
