...
class TemporalSpineHealthAndSidecarTests(unittest.TestCase):
    def test_health_includes_temporal_spine_aggregate(self):
        ...
        self.assertIn("temporal_spine_health()", src)
        self.assertIn('"temporal_spine": temporal_spine_health()', src)

    def test_public_maez_state_strips_temporal_spine(self):
        src = (_REPO / "skills" / "web_interface.py").read_text()
        route = src.split('@app.route("/api/maez-state")', 1)[1].split("@app.route", 1)[0]

        self.assertIn('daemon_health.pop("temporal_spine", None)', route)
    ...
    def test_sidecar_red_gates_missing_temporal_spine(self):
        ...
        self.assertIn("temporal_spine_unavailable", red_gates(sample))
