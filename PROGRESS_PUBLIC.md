...
- [ ] Tracks which observations led to the owner taking action vs. being ignored
...
- [ ] **Screen awareness** — Periodic screenshot capture (every 60s or on-demand), analyzed by a local vision model to understand what the owner is working on. "I see you have VS Code open with a Python file" → context-aware suggestions
...
- [ ] **Camera** — Presence detection using a lightweight face detection model. Is the owner at his desk? Adjust behavior: present → active monitoring, absent → reduced cycle frequency, conservation mode
- [ ] **Clipboard monitoring** — Capture clipboard changes to understand context. If the owner copies an error message, Maez can proactively research it
...
- [ ] Store the owner's skills, experience, and preferences in core memories
...
- [ ] Tracks the owner's approval/rejection patterns in core memory
...
- Presence detection — MediaPipe Tasks API (v0.10.33)
  ...
  Detects arrival/departure, greets the owner by name on arrival
...
| [SCREEN] | What the owner is working on (gemma4 vision) | Every 2 cycles |
| [CALENDAR] | Upcoming events, meeting alerts | Every 10 cycles |
| [PRESENCE] | Is the owner at his desk, session duration | Every 2 cycles |
...
1. Face recognition — Maez learns the owner's face specifically. Knows difference between the owner and others.
...
- Face recognition — the owner enrolled, 20 frames, 0.97 confidence
  Maez now knows the owner specifically vs any other person
...
[SCREEN] What the owner is working on (gemma4 vision)
[CALENDAR] Upcoming events, 8 hours ahead
[PRESENCE] the owner at desk, recognized by face, session duration
...
- [person] (the owner's partner) had first conversation with Maez via Maez_AI bot
- Maez remembered her throat infection across a session gap
- Maez discovered she is the owner's partner through natural conversation
...
1. Public-to-private memory bridge: public conversations summarized into the owner's awareness
...
- Streaming Telegram responses — `ollama.chat(stream=True)`, message edits every 20 tokens, the owner sees response forming in real-time
...
[GITHUB]           the owner's 14 repos + trending AI
...
- Memory isolation — guest users search only their own conversations, not the owner's archive
...
7. Clipboard monitoring — context from what the owner copies/pastes
...
Maez decides when to propose self-edits based on real cognition evidence from the daemon critique path. 7 conditions evaluated every critique window. AUTO_APPLY_ENABLED = False — the owner always approves. Trigger proven firing from the real daemon with live Ollama calls.
